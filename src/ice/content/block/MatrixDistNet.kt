...
object MatrixDistNet : Load {
  ...
  var 矩阵中枢伺服器 = DistNetCore("matrix_core").apply {
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris Merkez Sunucusu"
        description = "Bir matris ağının çekirdek cihazı, matris ağının bir ve yalnızca bir merkez sunucusu olmalıdır, matris köprüsü bu bloğa bağlanırken tek yönlü bağlanabilir"
      }
    }
    requirements(
  ...
  var 矩阵桥 = MatrixBridge("matrix_bridge").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris Köprüsü"
        description = "Matris tarafından kurulan madde taşıma köprüsü, herhangi bir yönde bağlanabilen madde taşıma köprüsü, eşya ve sıvı taşıyabilir\nAynı zamanda bu, matris ağındaki cihazları bağlamak için kullanılan düğümdür, matris ağı yalnızca ağaç yapısında kurulabilir"
      }
    }
  }
  var 矩阵塔 = MatrixBridge("matrix_tower").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris Kulesi"
        description = "Büyük matris bağlantı sütunu, matris köprüsünden daha uzun bağlantı mesafesine sahiptir, ancak yalnızca dört yöne bağlanabilir"
      }
    }
  }
  var 网格控制器 = MatrixGridCore("matrix_controller").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Izgara Denetleyici"
        description = "Matris ızgarasının kontrol merkezi, matris ızgarası çerçevesiyle matris ızgarası kurar, ızgarada bu cihaz aracılığıyla ızgara içi giriş çıkış noktalarını ve depolama cihazlarını yapılandırır, önemli bir lojistik yönetim modelidir"
      }
    }
  }
  var 网格框架 = MatrixEdgeBlock("matrix_grid_node").apply {
    localization {
      ...
      tr_TR {
        this.localizedName = "Izgara Çerçevesi"
        description = "Matris ızgarası yapı cihazları, birbirine bağlanır, bir ağ denetleyicisiyle kapalı döngü oluşturduktan sonra matris ızgarasını kurar"
      }
    }
    requirements(
      SglCategory.matrix, ItemStack.with(
  ...
  var 通用IO端口 = GenericIOPoint("io_point").apply {
    localization {
      ...
      tr_TR {
        this.localizedName = "Genel G/Ç Noktası"
        description = "Matris ızgarası tarafından kullanılan G/Ç tesisi, eşya ve sıvı için genel port, ızgara denetleyicisi aracılığıyla port yapılandırılır"
      }
    }
    requirements(
      SglCategory.matrix, ItemStack.with(
  ...
  var 能源管理器 = DistEnergyManager("matrix_energy_manager").apply {
    localization {
      ...
      tr_TR {
        this.localizedName = "Enerji Yöneticisi"
        description = "Enerji kabul ve yönetim cihazı, bitişik enerji cihazlarıyla bir enerji yönetim modülü oluşturur, ağ kullanımı için enerji kabulü ve önbelleğe alma için kullanılır"
      }
    }
    requirements(
      SglCategory.matrix, ItemStack.with(
  ...
  var 能源簇 = DistEnergyBuffer("matrix_energy_buffer").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Enerji Kümesi"
        description = "Ağ enerji önbellek cihazı, kesinti gibi durumlardan kaynaklanan ölümcül sorunları önlemek için belirli bir miktar matris enerjisi depolayabilir"
      }
    }
  ...
  var 能量接口 = DistPowerEntry("matrix_power_interface").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Enerji Arayüzü"
        description = "Enerji tedarik ağı tüketiminden cihaz, enerji yöneticisinin yakınına yerleştirilmelidir"
      }
    }
    consPower = 1000f
    eneProd = 480f
  ...
  var 中子接口 = DistNeutronEntry("matrix_neutron_interface").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Nötron Arayüzü"
        description = "Ağın nötron enerjisi girişini kabul eden cihaz, enerji yöneticisinin yakınına yerleştirilmelidir"
      }
    }
  }
  var 矩阵组件接口 = ComponentInterface("matrix_component_interface").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris Bileşen Arayüzü"
        description = "Matris fonksiyon bileşenlerini ağa bağlamak için kullanılır, birbirine bağlanarak sürekli bir arayüz yapısı oluşturması gerekir ve bileşen cihazlarının gerektirdiği bağlantı yöntemiyle eşleşmesi gerekir"
      }
    }
  }
  var 矩阵处理单元 = CoreNeighbourComponent("matrix_process_unit").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris İşlem Birimi"
        description = "Matris merkez sunucusunun yanına yerleştirilmesi gerekir, ağ çekirdeği hesaplama gücü sağlar, ağın her yenilemesinde daha fazla istek görevi işleyebilir"
      }
    }
  }
  var 矩阵拓扑容器 = CoreNeighbourComponent("matrix_topology_container").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Matris Topoloji Konteyneri"
        description = "Matris merkez sunucusunun yanına yerleştirilmesi gerekir, tüm ağın topoloji kapasitesini sağlar, ağın daha fazla cihaz kurmasına izin verir"
      }
    }
  }
  var 通用物质缓存器 = NetPluginComp("matrix_buffer").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Genel Madde Önbelleği"
        description = "Ağ bileşeni, ağın eşya ve sıvı önbellek alanı kapasitesini sağlar"
      }
    }
  }
  var 自动回收组件 = AutoRecyclerComp("automatic_recycler_component").apply {
    ...
    localization {
      ...
      tr_TR {
        this.localizedName = "Otomatik Geri Dönüşüm Bileşeni"
        description = "Önbellek taşma kaynak geri dönüşümü yapılandırılabilir, bu cihaz kurulu ağda, tüm ağ konteynerleri doluysa, yapılandırılmış geri dönüşüm listesindeki kaynaklar ağ tıkanmasını önlemek için otomatik olarak imha edilecektir\nİki yapılandırma modu vardır:\n    [accent]Kara liste modu[]: Taşan seçili kaynakları imha et\n    [accent]Beyaz liste modu[]: Taşma sırasında seçili kaynakları koru"
      }
    }
