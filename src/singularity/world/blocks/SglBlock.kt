...
open class SglBlock(name: String) :IceBlock(name), ConsumerBlockComp {
  ...
  override fun init() {
    ...
    for(cons in consume) {
      ...
      consumeEnergy = consumeEnergy or (cons.get(ConsumeType.energy) != null)
  ...
  override fun setStats() {
    ...
    if (hasEnergy) {
      ...
      stats.add(IceStats.最大能量势, energyCapacity, SglStatUnit.neutronFlux)
      ...
      if (activation >= 0.5f) {
        ...
        if (Mathf.chanceDelta((0.1f * Mathf.maxZero(activation - 0.5f)).toDouble())) {
          Angles.randLenVectors(
            ...
            SglParticleModels.floatParticle.create(this.x, this.y, SglDrawConst.fexCrystal, x, y, 2.6f).strength = 0.4f
          }
        }
      }
      updateEnergy()
        ...
        for(stack in displayLiquids) {
          bars.add(
            Bar(
              ...
              { min(liquids.get(stack.liquid) / block.liquidCapacity, 1f) })
          ).growX()
          bars.row()
        }
      }
