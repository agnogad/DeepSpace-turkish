...
class UnitFactoryCfgDialog : BaseDialog(Core.bundle.get("dialog.unitFactor.title")) {
                ...
                t.pane(Cons { list: Table ->
                    rebuild = Runnable {
                        ...
                        tmpList.sort(Comparator { a: BaseProducers?, b: BaseProducers? ->
                            val payloadsA = if (a!!.get(ProduceType.payload) == null) null else a.get(ProduceType.payload)!!.payloads[0]
                            val payloadsB = if (b!!.get(ProduceType.payload) == null) null else b.get(ProduceType.payload)!!.payloads[0]
                        ...
                        for (i in 0..<tmpList.size) {
                            ...
                            val cons = prod.cons
                            val payloads = if (prod.get(ProduceType.payload) == null) null else prod.get(ProduceType.payload)!!.payloads
                                                            ...
                                                            t.top().table(Cons { top: Table ->
                                                                ...
                                                                top.table(Cons { info: Table ->
                                                                    ...
                                                                    info.table(Cons { req: Table ->
                                                                        ...
                                                                        val ci = cons!!.get(ConsumeType.item)
                                                                        val cp = cons.get(ConsumeType.power)
                                                                        val cn = cons.get(ConsumeType.energy)
    ...
    fun rebuild(factory: SglUnitFactoryBuild) {
        ...
        status!!.add("").update(Cons { l: Label? ->
            ...
            if (factory.currentTask != null && factory.currentTask!!.factoryIndex != -1) {
                var cons = (factory.block as SglUnitFactory).consumers.get(factory.currentTask!!.factoryIndex) as BaseConsumers
                time += factory.currentTask!!.queueAmount * cons.craftTime - (factory.progress() + factory.buildCount()) * cons.craftTime

                for (task in factory.currentTask) {
                    if (task === factory.currentTask) continue

                    cons = (factory.block as SglUnitFactory).consumers.get(task.factoryIndex) as BaseConsumers
        ...
        if ((factory.block as SglUnitFactory).hasEnergy) {
            status!!.row()
            status!!.add("").update(Cons { l: Label? ->
                val cons = if (factory.consumer!!.current == null) null else factory.consumer!!.current!!.get(ConsumeType.energy)
                l!!.setText(
                    Core.bundle.format(
    ...
    private fun buildTaskItem(table: Table, task: BuildTask?, executing: Boolean) {
        ...
        mark[0] = table.table(SglDrawConst.grayUIAlpha, Cons { ta: Table ->
            ...
            val cons = (currConfig!!.block as SglUnitFactory).consumers.get(task.factoryIndex) as BaseConsumers
